...
from sklearn.metrics import accuracy_score, f1_score, matthews_corrcoef, r2_score, roc_auc_score
...
def train_rf_classifier(X_train, y_train, n_classes):
...
def eval_rf_classifier(model, X_test, y_test, n_classes):
...
def _first_metric_col(col_prefix: str, path: str) -> str:
    """Returns the first expected metric column for a given prefix."""
    df = load_lra_records(path)
    if df.empty or col_prefix not in "".join(df.columns):
        return f"{col_prefix}_MCC"
    # Find any column matching the prefix
    for col in df.columns:
        if col.startswith(col_prefix + "_"):
            return col
    return f"{col_prefix}_MCC"
...
def load_vep_coding(genome: Fasta, seq_len: int = 100_000) -> tuple:
    path = os.path.join(LRA_DIR, "variant_effect_pathogenic", "vep_pathogenic_coding.csv")
...
def main():
    ...
    print(f"Risultati: {LRA_RECORDS_CSV}\n")

    # Carica FM se richiesto
    fm = None
    if args.model:
    ...
    print(f"\n✓ Completato! Risultati in {LRA_RECORDS_CSV}")
