"""
Calcola la Mutual Information media tra feature e label per ogni dataset.

Per i k-mer: MI media su tutte le 4^k feature (frequenze k-mer).
Per i FM: MI media su tutte le D dimensioni dell'embedding.

Usa il training set per il calcolo.

Usage:
    python3 src/fm_experiment/compute_mi.py --k-values 4 5 6
    python3 src/fm_experiment/compute_mi.py --k-values 4 6 --fm-models InstaDeepAI/NTv3_650M_pre
    python3 src/fm_experiment/compute_mi.py --k-values 4 6 --fm-models LongSafari/hyenadna-medium-160k-seqlen-hf
"""
...
from src.fm_experiment.records import (
    load_records, has_mi_kmer, has_mi_fm, has_mi_tok,
    write_mi_kmer, write_mi_fm, write_mi_tok, RECORDS_CSV,
)
...
def main():
    ...
    parser.add_argument("--max-train-samples", type=int, default=50000,
                        help="Subsample training set se più grande (default 50000)")
    ...
    for ds in all_datasets:
        ...
        # Carica labels (sempre necessarie)
        train_seqs, train_labels, _, _ = load_dataset(ds["train_path"], ds["test_path"])

        if len(train_labels) > args.max_train_samples:
            n_skip = len(pending_ks) + len(pending_fms) + len(pending_toks)
            pbar.update(n_skip)
            pbar.set_description(f"⚠️  {name} (n={len(train_labels)} > {args.max_train_samples})")
            continue
