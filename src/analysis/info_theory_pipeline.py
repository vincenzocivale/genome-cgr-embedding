...
def _load_embeddings(model_name: str, dataset_name: str, split: str, kind: str) -> np.ndarray | None:
    """Load FM or tokenizer embeddings from cache. kind: 'fm' or 'tok'."""
    if kind == "fm":
        return load_cached_embeddings(model_name, dataset_name, split)
    # tokenizer embeddings use a separate cache directory
...
def main():
    ...
    parser.add_argument("--output", default="results/exploratory/info_theory.csv")
