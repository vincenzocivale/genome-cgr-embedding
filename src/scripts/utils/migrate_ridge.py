"""
One-shot migration: move ridge columns from records_rf.csv to records_decomposition.csv.

- Copies ridge_k{K}_* columns (single k only, excludes ridge_multi_*)
- Removes ALL ridge_* columns (including multi) from records_rf.csv
- Idempotent: safe to re-run if columns were already migrated.

Usage:
    python3 src/scripts/utils/migrate_ridge.py
"""
import os
import sys
import pandas as pd
...
DECOMP_CSV = "results/decomposition/records_decomposition.csv"
