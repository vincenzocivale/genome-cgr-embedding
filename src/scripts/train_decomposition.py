...
def _make_embedder(model_name: str, fm_batch_size: int):
...
def _cache_paths_for_dataset(model_name: str, dataset_name: str) -> list[str]:
...
def _delete_dataset_cache(model_name: str, dataset_name: str):
    ...
    for p in _cache_paths_for_dataset(model_name, dataset_name):
        if os.path.exists(p):
            os.remove(p)
    # Clean up empty parent dirs
    safe_model = model_name.replace("/", "__")
    safe_ds = dataset_name.replace("/", "__").replace("\\", "__")
    ds_dir = os.path.join("cache/fm_embeddings", safe_model, safe_ds)
    if os.path.isdir(ds_dir) and not os.listdir(ds_dir):
        os.rmdir(ds_dir)
    if "hyenadna" in model_name.lower():
        ds_dir_h = os.path.join("cache/hyena_embeddings", safe_ds)
...
def main():
    ...
    for ds in all_datasets:
        ...
        for k in args.k_values:
            need_ridge = not has_decomp_ridge(name, k, args.model, records)
            need_proj = not has_proj(name, k, args.model, records)
            need_resid = not has_resid(name, k, args.model, records)
        ...
        Y_train = load_cached_embeddings(args.model, name, "train")
        Y_test = load_cached_embeddings(args.model, name, "test")
        generated_embeddings = False

        if Y_train is None or Y_test is None:
            pbar.set_description(f"[embedding] {name}")
            if embedder is None:
                embedder = _make_embedder(args.model, args.fm_batch_size)
        ...
        for k, need_ridge, need_proj, need_resid in pending_ks:
            ...
            # Save Ridge metrics
            if need_ridge:
                write_decomp_ridge(name, k, args.model, {
        ...
        if not args.no_cleanup and generated_embeddings:
            _delete_dataset_cache(args.model, name)
