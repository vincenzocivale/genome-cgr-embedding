...
LRA_RECORDS_CSV = "results/regression/lra_records_rf.csv"
...
def load_vep_coding(genome: Fasta, seq_len: int = 100_000) -> tuple:
    _raid_path = "/raid/DATASETS/dna_foundation_benchmark/pathogenic/vep_pathogenic_coding.csv"
    _local_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "..", "data",
        "dna_foundation_benchmark", "pathogenic", "vep_pathogenic_coding.csv"
    )
    if os.path.exists(_raid_path):
        path = _raid_path
    else:
        path = os.path.normpath(_local_path)
